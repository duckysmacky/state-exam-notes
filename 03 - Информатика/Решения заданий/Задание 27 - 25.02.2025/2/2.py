...
def disperse(cluster, b_avg):
    return sum((b - b_avg) ** 2 for x, y, b in cluster) / len(cluster)

def count_special(cluster):
    b_avg = sum(b for x, y, b in cluster) / len(cluster)
    deviation = disperse(cluster, b_avg) ** 0.5
    special = [b for x, y, b in cluster if abs(b - b_avg) > deviation]
    return len(special)
...
special = [count_special(cluster) for cluster in clusters]

P_xy = sum(x + y for x, y, b in centroids) / len(centroids)
P_n = sum(special)
print(int(P_xy * 10_000), int(P_n))
